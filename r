...
class Reminder
  ...
  DEFAULT_REMINDERS_FILE = "#{ENV['HOME']}/.reminders"
  ...
  def hash
    (Digest::SHA2.new << data.to_s).to_s
  end
  ...
  class << self
    ...
    def destroy(reminders_to_destroy, file = nil)
    ...
    # make sure the ~/.reminders file exists, if not, create it
    def init
      unless File.exist? DEFAULT_REMINDERS_FILE then
        FileUtils.touch DEFAULT_REMINDERS_FILE
      end
    end
  end
  ...
  def self.raw_clear_and_write(reminders, file)
